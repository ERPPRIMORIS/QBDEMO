...
#                                                 print("amount is ***********",dict_t.get('Amount'))
                                        ...
                                        now = datetime.now()
                                        dict_s['confirmation_date'] = now.strftime("%Y-%m-%d %H:%M:%S")
        ...
        if cust.get('Line'):
            for lines in cust.get('Line'):
                if 'SalesItemLineDetail' in lines and lines.get('SalesItemLineDetail').get('ItemRef').get('value'):
                    ...
                    res_product = self.env['product.product'].search([('qbo_product_id', '=', lines.get('SalesItemLineDetail').get('ItemRef').get('value'))])
                    if res_product:
...
# #
    ...
    def import_invoice(self):
        ...
        if company.access_token:
            ...
            query = "select * from invoice WHERE Id > '%s' order by Id" % (company.quickbooks_last_invoice_imported_id)
            ...
            if data:
                ...
                if parsed_data:
                    if parsed_data.get('QueryResponse') and parsed_data.get('QueryResponse').get('Invoice'):
                        for cust in parsed_data.get('QueryResponse').get('Invoice'):
                            ...
                            if return_val:
                                ...
                                if not account_invoice:
                                    ...
                                    if res_partner:
                                        ...
                                        if cust.get('DueDate'):
                                            dict_i['date_due'] = cust.get('DueDate')
                                        if cust.get('TxnDate'):
                                            dict_i['date_invoice'] = cust.get('TxnDate')
...
#                                                 dict_i['amount'] = dict_t.get('Amount')
                                        ...
                                        invoice_obj = self.env['account.invoice'].create(dict_i)
...
#                                             self._cr.commit()
                                        ...
                                        for i in cust.get('Line'):
                                            ...
                                            if i.get('SalesItemLineDetail'):
                                                ...
                                                if res_product:
                                                    ...
                                                    dict_ol.clear()
                                                    dict_ol['invoice_id'] = invoice_obj.id
                                                    ...
                                                    if i.get('SalesItemLineDetail').get('TaxCodeRef'):
                                                        ...
                                                        if tax_val == 'TAX':
                                                            ...
                                                            dict_ol['invoice_line_tax_ids'] = 0
                                                    ...
                                                    if 'account_id' in dict_ol:    
                                                        ...
                                                        create_p = self.env['account.invoice.line'].create(dict_ol)
                                                        if create_p:
                                                            self._cr.commit()
                                                            _logger.info("Invoice Line Committed!!!")
                                                            create_p.invoice_id._onchange_invoice_line_ids()
                                        ...
                                        if cust.get('Balance') == 0:
                                            if invoice_obj.state == 'draft':
                                                ...
                                                if cust.get('DocNumber'):
                                                    invoice_obj.write({'number': cust.get('DocNumber'),
                                                                       'residual': cust.get('Balance'),
                                                                       'residual_signed': cust.get('Balance')})
...
#                                                 dict_i['amount'] = dict_t.get('Amount')
                                        ...
                                        for i in cust.get('Line'):
                                            ...
                                            if i.get('SalesItemLineDetail'):
                                                ...
                                                if res_product:
                                                    p_order_line = self.env['account.invoice.line'].search(
                                                        ['&', ('product_id', '=', res_product.id),
                                                    ...
                                                    if p_order_line:
                                                        ...
                                                        # create_p = self.env['account.invoice.line'].write(dict_ol)

                                                        create_iv = self.env['account.invoice.line'].search(
                                                            ['&', ('qb_id', '=', int(i.get('Id'))),
                                                             (('invoice_id', '=', account_invoice.id))])
                                                        ...
                                                        if create_iv:
                                                            res = create_iv.write({
                                                                ...
                                                                'qb_id': ol_qb_id,
                                                                'price_unit': sp,
                                                                'invoice_line_tax_ids': custom_tax_id_id,
                                                            })
    ...
    #                                                     print("**********PRODUCT ACCOUNT CHECK************",res_product_acc.property_account_income_id)
                                                        dict_ol.clear()
                                                        dict_ol['invoice_id'] = account_invoice.id
                                                        dict_ol['product_id'] = res_product.id
                ...
                if parsed_data:
                    ...
                    if parsed_data.get('QueryResponse') and parsed_data.get('QueryResponse').get('Bill'):
                        ...
                        for cust in parsed_data.get('QueryResponse').get('Bill'):
                            ...
                            bill = self.env['account.invoice'].search(
                                [('qbo_invoice_id', '=', cust.get('Id'))])
...
#                                             dict_i['amount'] = dict_t.get('Amount')
                                    ...
                                    for i in cust.get('Line'):
                                        ...
                                        if i.get('AccountBasedExpenseLineDetail'):
                                            dict_al = {}
                                            dict_al['invoice_id'] = invoice_obj.id
                                            if i.get('Id'):
                                                dict_al['qb_id'] = int(i.get('Id'))
                                                dict_al['invoice_line_tax_ids'] = 0
                                                dict_al['quantity'] = 1
                                            ...
                                            create_p = self.env['account.invoice.line'].create(dict_al)
                                            if create_p:
                                        ...
                                        if i.get('AccountBasedExpenseLineDetail'):
                                            account_account = self.env['account.account'].search([('qbo_id', '=',
                                                                                                   i.get(
                                                                                                       ...
                                                                                                       'value'))])
                                            if account_account:
                                                a_order_line = self.env['account.invoice.line'].search(
                                                ...
                                                if not a_order_line:
                                                    ...
                                                    _logger.info("Redirecting else part account.invoice.line")
                                                    create_p = self.env['account.invoice.line'].write(dict_al)
                                                    if create_p:
...
#                                                 dict_i['amount'] = dict_t.get('Amount')
    ...
    def export_customers(self):
        ...
        for contact in res_partner:
            if contact.id == 1 or contact.id == 3:
                ...
                if contact.customer == True and contact.type == 'contact':
    ...
    def export_payment_method(self):
        payment_method = self.env['qbo.payment.method'].search([])
