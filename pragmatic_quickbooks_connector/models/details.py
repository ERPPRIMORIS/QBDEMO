        ...
        if res.check:
            if res.discount_type and res.amount:
                ...
                if res.discount_type == 'percentage':
                    ...
                    acc_inv_line_id = self.env['account.invoice.line'].create(acc_inv_line)
                else:
                    acc_inv_line = {
                        'product_id': res_product.id,
                        'account_id': account.id,
        ...
        if vals.get('check'):
            if vals.get('discount_type') and vals.get('amount'):
                if vals.get('qbo_invoice_id'):
                    ...
                    account_invoice = self.env['account.invoice'].search([('qbo_invoice_id','=',qb_id)])
                ...
                product = self.env['product.product'].search([('is_discount_product', '=', True)], limit=1)
                discount_line = self.env['account.invoice.line'].search([('product_id','=',product.id),('invoice_id','=',account_invoice.id)],limit=1)
...
class InvoiceLine(models.Model):
    ...
    @api.model
    def create(self,vals):
        _logger.info("ACC INV Vals :--------------------> {} ".format(vals))
