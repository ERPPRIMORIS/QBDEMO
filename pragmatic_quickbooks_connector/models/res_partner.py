...
class ResPartner(models.Model):
    ...
    def _prepare_partner_dict(self, partner, is_customer=False, is_vendor=False):
        ...
        if partner.get('GivenName') and partner.get('MiddleName') and partner.get('FamilyName'):
            vals.update({'name': partner.get('GivenName') + ' ' + partner.get('MiddleName') + ' ' + partner.get('FamilyName')})
...
#         child_ids = []
        if 'BillAddr' in partner and partner.get('BillAddr'):
            address_vals = {
                ...
                'state_id': self.env['res.country.state'].get_state_ref(partner.get('BillAddr').get('CountrySubDivisionCode'), partner.get('BillAddr').get('Country')) if partner.get('BillAddr').get('CountrySubDivisionCode') and partner.get('BillAddr').get('Country') else False,
    ...
    def checkPartnerInQuickbooks(self, odoo_partner_object):
        ...
        if company:
            ...
            if access_token and realmId:
                ...
                if odoo_partner_object.supplier:
        ...
        if len(self) > 1:
            for customer in self:
                if customer.type == 'contact':
                    ...
                    elif customer.supplier == True:
                        customer.exportVendor()
        else:
            if self.type == 'contact':
                if self.customer == True:
                    self.exportCustomer()
                elif self.supplier == True:
                    self.exportVendor()
